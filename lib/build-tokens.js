...
async function buildTokensCommand(commandArgs) {
  ...
  appsToProcess.forEach(appName => {
    configs.push({ config: getAppStyleDictionaryConfig(appName), isApp: true, appName });
  });

  // Used to partition a theme's tokens into "from core" vs "from theme".
  const paragonCoreDir = path.resolve(__dirname, '../tokens/src/core');
  const userCoreDir = tokensSource ? path.resolve(tokensSource, 'core') : null;
  const isFromCore = (filePath) => (
    filePath.startsWith(`${paragonCoreDir}${path.sep}`)
    || (userCoreDir && filePath.startsWith(`${userCoreDir}${path.sep}`))
  );

  // Phase 1: build all token configs (core, themes, apps). Serial so that
  // each verbose stats line lands right after its own css build output.
  // eslint-disable-next-line no-restricted-syntax
  for (const { config, themeVariant, isApp, appName } of configs) {
    ...
    if (verbose) {
      ...
      if (isApp) {
        const sourceCount = sd.allTokens.filter(t => t.isSource).length;
        // eslint-disable-next-line no-console
        console.log(`\napp (${appName}): processed ${sourceCount} tokens`);
      } else if (themeVariant) {
        const coreCount = sd.allTokens.filter(t => isFromCore(t.filePath)).length;
        const themeCount = total - coreCount;
        // eslint-disable-next-line no-console
        console.log(`\ntheme (${themeVariant}): processed ${total} tokens (${coreCount} core, ${themeCount} theme)`);
      } else {
        // eslint-disable-next-line no-console
        console.log(`\ncore: processed ${total} tokens`);
      }
    }
  }
